...
Every supervised lesson before L06 assumed rows were independent — you could shuffle them, k-fold cross-validate, and the order didn't matter. Sarah's daily revenue file breaks that assumption: today's number depends on yesterday's, on the same day last week, and on the same day last year. By Friday she will hold four new tools: **STL decomposition** (to separate trend, seasonality, and noise), **classical forecasting with ETS/Holt-Winters** (the pragmatic baseline every team should run first), **ML forecasting with lag features** (the modern industry default when you have exogenous signals or multiple seasonalities), and **time-series cross-validation** (the only honest way to evaluate any of them). Every future-facing number she ever ships — revenue forecasts, demand plans, capacity decisions — passes through these four ideas.

---

## Key takeaways

1. **Time series breaks the i.i.d. assumption.** Today depends on yesterday, last week, and last year. That single fact rules out random shuffling, k-fold cross-validation, and any pipeline that lets future data leak into the past.
...
7. **Use TimeSeriesSplit, never KFold.** Each fold's test window must come *after* its training window. The final fold (most data, most recent test) is your most realistic estimate of deployment performance.
...
3. **Was the evaluation time-respecting?** Random k-fold leaks the future into the past. Use `TimeSeriesSplit`, report the last fold's metric (closest to deployment conditions), and quote a range, not a single point — forecast uncertainty grows with the horizon.
...
**Q5.** Why not just k-fold cross-validate the forecast model?
...
**Q6.** ETS converged poorly and gave a warning. Should you trust the forecast?
...
**Q7.** Your GB forecast has RMSE = £500 in training and RMSE = £2000 on a 30-day held-out test. What's happening?
...
**Q8.** Should you scale lag features before feeding them to HistGradientBoosting?

> *Sample answer:* Trees don't care about scale. You don't need to scale for tree-based models. (For neural networks, yes.)

**Q9.** Your ML forecaster includes `lag_1`, `lag_7`, `lag_30`. Should you add `lag_2`, `lag_3`, ...?
